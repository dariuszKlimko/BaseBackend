import { Body, Controller, Delete, Get, InternalServerErrorException, NotFoundException, Param, Patch, Post, UseFilters, UseGuards } from "@nestjs/common";
...
export class MeasurementsController {
    ...
    @Post()
    async createMeasurement(@CurrentUser() user: CurrentUserDecorator, @Body() measurement: CreateMeasurementDto): Promise<MeasurementResponseDto> {
        try{
    ...
    @UseFilters(HttpExceptionFilter)
    @Get("/:id")
    async getOneMeasurement(@CurrentUser() user: CurrentUserDecorator, @Param() measurementId: MeasurementId): Promise<MeasurementResponseDto> {
        try{
    ...
    @UseFilters(HttpExceptionFilter)
    @Patch("/:id")
    async updateMeasurement(@CurrentUser() user: CurrentUserDecorator, @Param() measurementId: MeasurementId, @Body() measurement: UpdateMeasurementDto): Promise<MeasurementResponseDto> {
        try{
    ...
    @UseFilters(HttpExceptionFilter)
    @Delete("/:id")
    async deleteOneMeasurement(@CurrentUser() user: CurrentUserDecorator, @Param() measurementId: MeasurementId): Promise<MeasurementResponseDto> {
        try{
